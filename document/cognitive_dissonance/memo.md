...
# Implicit Choice Paradigm
参加者は実験室に到着し、実験者が出迎えた。研究は2回のグループセッション（それぞれ21名と19名）で行われ、各参加者は個別の孤立したワークステーションに着席した。提示刺激は、80の異なる国名（例：オーストラリア、ブラジル、スウェーデン）からなり、休暇の目的地として設定された（Sharot et al.、2010に準拠）。実験はz-Tree（Fischbacher, 2007）を用いてコンピュータで実施され、選択前評価課題（R1）、選択課題、選択後評価課題（R2）から構成されている。評価タスクの質問は、"来年の休暇をこの旅行先で過ごすとしたら、どの程度嬉しいか？"であった。タスクR1では、参加者は選択肢を単極の8点評価尺度（1＝少し嬉しいから8＝とても嬉しいまで）で評価した。各目的地はコンピュータ画面上に6秒間提示された(ランダムな順序で)。その後、評価スケールが目的地の下に表示され、参加者は20秒間で評価を提出した。時間的プレッシャーの影響を避けるため、先行研究よりも1回の試行に与える時間を長くした。
選択課題では、参加者は画面上に16組の目的地を順次提示され、各組の2つの目的地から（仮説的に）選択するよう求められた。参加者は1回の決断に20秒を費やした。ペアと画面上の刺激の位置（左右）は完全にランダム化され、選択肢は2度使われなかった。選択課題のペアは、等しく評価された目的地（すなわち、評価距離D=0）のペア（a,b）、できれば中心評価スコア4または5を探索するアルゴリズムによって決定された。このような4組のペアを用いて直接選択ペアを構成した。残りの(a,b)ペアについて，a,bより評価の高い目的地hと低い目的地ℓを探索した．目的地は対称に選ばれ，すなわち，構築されたペアにおける選択肢間の距離は，ペアにおける距離と等しく，距離はD = 1からD = 3まで変化した．このアルゴリズムは、6組の(a,h)と6組の(b,ℓ)の組、つまり合計6組の暗黙の組をランダムに構成しようとするものであった。21名の参加者において、これは可能であった。10人の参加者は5組の暗黙のペアを構築することができ、さらに5人の参加者は3組または4組のペアしか見つけることができなかった。このようなペアが6つ以下しか見つからなかった場合、アルゴリズムは、各参加者がちょうど16の選択肢に直面するように、等しく評価された選択肢の直接のペア（a、b）をさらに追加しました。直接選択ペアはコントロールとして含まれ、古典的なFCPに相当する。選択後の評価課題(R2)はR1と同じであった。参加者は再び80個すべての刺激をランダムな順序で提示され、それらを再評定した。一貫性効果を避けるため、この課題は記憶課題でないことが明示された。
